...
def main():
    ...
    url_mappings = {
        "https://canvas.acu.edu.au/courses/28429": "https://canvas.acu.edu.au/courses/28429",
        "https://canvas.acu.edu.au/courses/28428": "https://canvas.acu.edu.au/courses/28428",
    ...
    try:
        subaccount_id = input("Please enter the subaccount ID: ")  # Prompt for subaccount ID
        # Get courses in the subaccount
        courses = scanner.make_request(f"accounts/{subaccount_id}/courses")
        
        # Use ThreadPoolExecutor for parallel processing of courses
        with concurrent.futures.ThreadPoolExecutor() as executor:
            futures = {executor.submit(scanner.scan_and_replace, course['id']): course for course in courses}
            for future in concurrent.futures.as_completed(futures):
                course = futures[future]
                try:
                    future.result()  # Get the result of the future
                    print(f"Successfully processed course: {course['name']} (ID: {course['id']})")
                except Exception as e:
                    print(f"Error processing course {course['name']} (ID: {course['id']}): {str(e)}")
